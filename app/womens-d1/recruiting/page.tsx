...
function divLabel(div: string) { return div === 'D1 Women' ? 'D1' : 'D2'; }
...
export default function WomensTransfersPage() {
  ...
  const [divFilter, setDivFilter]   = useState<'all' | 'D1 Women' | 'D2 Women'>('all');
  ...
  const exportCSV = () => {
    ...
    a.href = url; a.download = 'womens_transfers.csv'; a.click();
                    ...
                    return (
                      <tr key={`${t.name}-${t.previousSchool}`} style={{ borderBottom: '1px solid #f0f0f0', background: bg }}>
                        ...
                        <td style={{ padding: '5px 8px', textAlign: 'center' }}>
                          <span style={{ display: 'inline-block', padding: '2px 6px', borderRadius: 4, fontSize: 10, fontWeight: 700, background: t.division==='D1 Women' ? NAVY : SKY, color: '#fff' }}>
                            {divLabel(t.division)}
                          </span>
                        </td>
