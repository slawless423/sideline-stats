...
export default function HSPlayerProfile() {
  ...
  // Usage — uses actual team minutes from DB (team.mp), not a derived constant.
  let usagePct = '—';
  if (team && player.mp > 0 && team.mp > 0) {
  ...
  // OR% — same substitution: use team.mp.
  let orPct = '—';
  if (team && player.mp > 0 && team.mp > 0) {
    // Approximate opp_drb using team totals if available on the fetched row;
    // the team list endpoint returns opp_* but we only typed a subset above.
    // Fall back gracefully when opponent rebounds aren't present.
    const teamAny = team as unknown as { opp_reb?: number; opp_oreb?: number };
    if (teamAny.opp_reb != null && teamAny.opp_oreb != null) {
      const opp_drb = teamAny.opp_reb - teamAny.opp_oreb;
      const denom = team.oreb + opp_drb;
      if (denom > 0) {
        const teamMinutes = team.mp;
        orPct = (((player.oreb / player.mp) * (teamMinutes / 5) / denom) * 100).toFixed(1);
      }
    }
  }
  ...
  return (
    <>
      ...
      <main style={{ maxWidth: 900, margin: '0 auto', padding: 24 }}>
        ...
        {/* Back link */}
        <Link href="/womens-d1/recruiting/highschool" style={{ fontSize: 12, color: MUTED, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 4, marginBottom: 20 }}>
          ← Back to High School
        </Link>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: 10, marginBottom: 28 }}>
          ...
          <StatBox label="OR%" value={orPct === '—' ? '—' : orPct + '%'} />
