...
function buildPlayerId(teamId, p, playerSeasonStats) {
  ...
  // Look for existing player on same team with same last name
  for (const [existingId, existing] of playerSeasonStats) {
    ...
    // Same last + same first = same person
    if (existingFirst === first) return existingId;

    // Same last + different first + same jersey number = same person
...
async function main() {
  ...
  console.log(`✅ WROTE public/data/womens_d1_games_cache.json (${allGames.length} games)`);
