...
function isD2Conference(conf) {
  return conf && WOMENS_D2_CONFERENCES.has(conf.toLowerCase());
}

console.log("START build_womens_d2_complete", new Date().toISOString());
...
function extractConferenceFromGame(gameObj) {
  try {
    ...
    return { gameId: String(gameId), homeConf, awayConf, isConferenceGame: homeConf && awayConf && homeConf === awayConf };
  } catch { return {}; }
}
...
async function main() {
  ...
  let days = 0, totalGamesFound = 0, totalBoxesFetched = 0, totalBoxesParsed = 0, totalBoxesFailed = 0;
  ...
  for (let dt = start; dt <= end; dt = addDays(dt, 1)) {
    ...
    const boxscoreFetches = await mapLimit(gameIds, BOX_CONCURRENCY, async (gid) => {
      try {
        ...
        if ((globalThis.__BOX_FAILS__ ?? 0) < 10) { globalThis.__BOX_FAILS__ = (globalThis.__BOX_FAILS__ ?? 0) + 1; console.log("boxscore fetch failed for gid:", gid); }
        await sleep(BOX_DELAY_MS);
    ...
    for (const { gid, box, date } of boxscoreFetches) {
      ...
      if (totalBoxesFetched === 1) {
        ...
        await fs.writeFile("public/data/womens_d2_sample_boxscore.json", JSON.stringify(box, null, 2), "utf8");
      ...
      if (confInfo) { gameData.home.conference = confInfo.homeConf; gameData.away.conference = confInfo.awayConf; gameData.isConferenceGame = confInfo.isConferenceGame; }
  ...
  for (const game of allGames) {
    ...
    const gameLogEntry = {
      ...
      isConferenceGame: game.isConferenceGame, players: [],
    };
    ...
    if (game.players && Array.isArray(game.players)) {
      for (const playerData of game.players) {
        ...
        const simplifiedPlayers = playerData.players.map((p) => ({
          ...
          division: DIVISION, firstName: p.firstName || "", lastName: p.lastName || "", number: p.number || "",
    ...
    for (const team of [home, away]) {
      ...
      if (!teamSeasonStats.has(team.teamId)) {
        teamSeasonStats.set(team.teamId, {
          ...
          opp_fgm: 0, opp_fga: 0, opp_tpm: 0, opp_tpa: 0, opp_ftm: 0, opp_fta: 0, opp_orb: 0, opp_drb: 0, opp_trb: 0, opp_ast: 0, opp_stl: 0, opp_blk: 0, opp_tov: 0, opp_pf: 0,
        });
      }
      const s = teamSeasonStats.get(team.teamId);
      s.games++; if (team.stats.points > oppStats.points) s.wins++; else s.losses++;
      s.points += team.stats.points; s.fgm += team.stats.fgm; s.fga += team.stats.fga; s.tpm += team.stats.tpm; s.tpa += team.stats.tpa; s.ftm += team.stats.ftm; s.fta += team.stats.fta; s.orb += team.stats.orb; s.trb += team.stats.trb; s.drb += Math.max(0, team.stats.trb - team.stats.orb); s.ast += team.stats.ast; s.stl += team.stats.stl; s.blk += team.stats.blk; s.tov += team.stats.tov; s.pf += team.stats.pf;
      s.opp_points += oppStats.points; s.opp_fgm += oppStats.fgm; s.opp_fga += oppStats.fga; s.opp_tpm += oppStats.tpm; s.opp_tpa += oppStats.tpa; s.opp_ftm += oppStats.ftm; s.opp_fta += oppStats.fta; s.opp_orb += oppStats.orb; s.opp_trb += oppStats.trb; s.opp_drb += Math.max(0, oppStats.trb - oppStats.orb); s.opp_ast += oppStats.ast; s.opp_stl += oppStats.stl; s.opp_blk += oppStats.blk; s.opp_tov += oppStats.tov; s.opp_pf += oppStats.pf;
    }
        ...
        for (const p of playerData.players) {
          ...
          if (!playerSeasonStats.has(playerId)) {
            playerSeasonStats.set(playerId, {
              ...
              games: 0, starts: 0, minutes: 0, fgm: 0, fga: 0, tpm: 0, tpa: 0, ftm: 0, fta: 0, orb: 0, drb: 0, trb: 0, ast: 0, stl: 0, blk: 0, tov: 0, pf: 0, points: 0,
            });
          }
          ...
          if (mins > 0 || parseInt(p.points || 0) > 0) {
            pStats.games++; if (p.starter === true || p.starter === "true") pStats.starts++;
            pStats.minutes += mins; pStats.fgm += parseInt(p.fieldGoalsMade || 0); pStats.fga += parseInt(p.fieldGoalsAttempted || 0); pStats.tpm += parseInt(p.threePointsMade || 0); pStats.tpa += parseInt(p.threePointsAttempted || 0); pStats.ftm += parseInt(p.freeThrowsMade || 0); pStats.fta += parseInt(p.freeThrowsAttempted || 0); pStats.orb += parseInt(p.offensiveRebounds || 0); pStats.trb += parseInt(p.totalRebounds || 0); pStats.drb += Math.max(0, parseInt(p.totalRebounds || 0) - parseInt(p.offensiveRebounds || 0)); pStats.ast += parseInt(p.assists || 0); pStats.stl += parseInt(p.steals || 0); pStats.blk += parseInt(p.blockedShots || 0); pStats.tov += parseInt(p.turnovers || 0); pStats.pf += parseInt(p.personalFouls || 0); pStats.points += parseInt(p.points || 0);
          }
        }
      }
    }
  ...
  for (const [teamId, stats] of teamSeasonStats) {
    ...
    ratingsRows.push({ teamId, team: stats.teamName, conference: stats.conference, games: stats.games, adjO: (stats.points / offPoss) * 100, adjD: (stats.opp_points / defPoss) * 100, adjEM: ((stats.points / offPoss) - (stats.opp_points / defPoss)) * 100, adjT: offPoss / Math.max(1, stats.games) });
  }
  ...
  await fs.mkdir("public/data", { recursive: true });
  await fs.writeFile("public/data/womens_d2_ratings.json", JSON.stringify({ generated_at_utc: new Date().toISOString(), season_start: SEASON_START, rows: ratingsRows }, null, 2), "utf8");
  console.log(`✅ WROTE public/data/womens_d2_ratings.json (${ratingsRows.length} teams)`);
  await fs.writeFile("public/data/womens_d2_team_stats.json", JSON.stringify({ generated_at_utc: new Date().toISOString(), teams: Array.from(teamSeasonStats.values()) }, null, 2), "utf8");
  console.log(`✅ WROTE public/data/womens_d2_team_stats.json`);
  await fs.writeFile("public/data/womens_d2_games.json", JSON.stringify({ generated_at_utc: new Date().toISOString(), games: gamesLog }, null, 2), "utf8");
  console.log(`✅ WROTE public/data/womens_d2_games.json (${gamesLog.length} games)`);
  await fs.writeFile("public/data/womens_d2_games_cache.json", JSON.stringify({ generated_at_utc: new Date().toISOString(), note: "Contains all successfully parsed game IDs", total_games: allGames.length, game_ids: allGames.map(g => g.gameId) }, null, 2), "utf8");
  console.log(`✅ WROTE public/data/womens_d2_games_cache.json (${allGames.length} games)`);
  await fs.writeFile("public/data/womens_d2_player_stats.json", JSON.stringify({ generated_at_utc: new Date().toISOString(), players: allPlayers }, null, 2), "utf8");
  ...
  if (process.env.POSTGRES_URL) {
    ...
    try {
      db.initDb();
      if (allGames.length < 500) throw new Error(`BAD RUN: Only ${allGames.length} games parsed. API may be down. Aborting to protect existing database data.`);
      ...
      for (const [teamId, teamStat] of teamSeasonStats) {
        ...
        if (row && teamStat) {
          await db.upsertTeam({ teamId: row.teamId, teamName: row.team, conference: row.conference, division: DIVISION, games: row.games, wins: teamStat.wins, losses: teamStat.losses, adjO: row.adjO, adjD: row.adjD, adjEM: row.adjEM, adjT: row.adjT, points: teamStat.points, opp_points: teamStat.opp_points, fgm: teamStat.fgm, fga: teamStat.fga, tpm: teamStat.tpm, tpa: teamStat.tpa, ftm: teamStat.ftm, fta: teamStat.fta, orb: teamStat.orb, drb: teamStat.drb, trb: teamStat.trb, ast: teamStat.ast, stl: teamStat.stl, blk: teamStat.blk, tov: teamStat.tov, pf: teamStat.pf, opp_fgm: teamStat.opp_fgm, opp_fga: teamStat.opp_fga, opp_tpm: teamStat.opp_tpm, opp_tpa: teamStat.opp_tpa, opp_ftm: teamStat.opp_ftm, opp_fta: teamStat.opp_fta, opp_orb: teamStat.opp_orb, opp_drb: teamStat.opp_drb, opp_trb: teamStat.opp_trb, opp_ast: teamStat.opp_ast, opp_stl: teamStat.opp_stl, opp_blk: teamStat.opp_blk, opp_tov: teamStat.opp_tov, opp_pf: teamStat.opp_pf });
        }
      }
        ...
        if (game.players && Array.isArray(game.players)) {
          for (const teamData of game.players) {
            ...
            for (const p of teamData.players) {
              playerGameRows.push({ gameId: game.gameId, playerId: p.playerId, teamId: teamData.teamId, division: p.division || DIVISION, minutes: p.minutes, fgm: p.fgm, fga: p.fga, tpm: p.tpm, tpa: p.tpa, ftm: p.ftm, fta: p.fta, orb: p.orb, drb: p.drb, trb: p.trb, ast: p.ast, stl: p.stl, blk: p.blk, tov: p.tov, pf: p.pf, points: p.points });
            }
          }
        }
      }
